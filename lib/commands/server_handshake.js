...
ServerHandshake.prototype.readClientReply = function(packet, connection) {
  // check auth here
  connection.clientHelloReply = new Packets.HandshakeResponse.fromPacket(packet);
  var clientHelloReply = connection.clientHelloReply;
...
ServerHandshake.prototype.dispatchCommands = function(packet, connection) {
  ...
  case CommandCode.FIELD_LIST:
    ...
    break;
  case CommandCode.CHANGE_USER:
    if (connection.listeners('change_user').length === 0) {
      connection.writeOk();
    } else {
      connection.emit('change_user');
    }
    break;
  default:
