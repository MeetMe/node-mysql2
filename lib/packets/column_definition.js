...
ColumnDefinition.toPacket = function(column, sequenceId)
{
  ...
  fields.forEach(function (name) {
    length += Packet.lengthCodedStringLengthInBytes(column[name]);
  });
  length += (4 + 13); // Header + trailer bytes
  ...
  packet.writeInt16(('characterSet' in column) ? column.characterSet : 0);
  packet.writeInt32(column.columnLength);
  packet.writeInt8(column.columnType);
  packet.writeInt16(('flags' in column) ? column.flags : 0);
  packet.writeInt8(('decimals' in column) ? column.decimals : 0);
  return packet;
};
